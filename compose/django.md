...
2. Create the Django project using the [docker-compose run](/compose/reference/run/) command.

    On Mac, run the command as follows:

        $ sudo docker-compose run web django-admin startproject composeexample .
        WARNING: The CURRENT_UID variable is not set. Defaulting to a blank string.
        Starting web_db_1 ... done
    
    On Windows, use command-prompt or powershell with the following command:
    ...
    On Linux, run the following:
        ...
        $ docker-compose CURRENT_UID=$(id -u):$(id -g) run web django-admin startproject composeexample .
        Starting web_db_1 ... done
    ...
    The `CURRENT_UID` variable will be set to blank automatically when using
    the Windows and Mac commands, but will be set to current user with the
    Linux command.

3. After the `docker-compose` command completes, list the contents of your project.

        $ ls -l
        drwxr-xr-x 2 user   user   composeexample
        -rw-rw-r-- 1 user   user   docker-compose.yml
        -rw-rw-r-- 1 user   user   Dockerfile
        -rwxr-xr-x 1 user   user   manage.py
